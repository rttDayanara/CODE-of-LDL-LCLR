function [spara,fval,exitFlag,output,grad] = lclldtrains(funfcn,xInit,optim)
...
OPTIONS.MaxIter=100;
% fprintf('Begin training of BFGS-LCLLD-S. \n');
...
if (~exist('optim','var')) 
    ...
    [spara,fval,exitFlag,output,grad] = fminlbfgs(funfcn,xInit);

else
    % Function is written by D.Kroon University of Twente (Updated Nov.2010).
    [spara,fval,exitFlag,output,grad] = fminlbfgs(funfcn, xInit,optim);
end
end
